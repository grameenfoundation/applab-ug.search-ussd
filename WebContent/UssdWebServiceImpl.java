...
public class UssdWebServiceImpl implements UssdWebServiceInterface {
    ...
    private UssdResponse getPreviousMenu(UssdRequest request, UssdMenu displayedMenu) throws Exception {
        UssdResponse appResp = new UssdResponse();
        isCompleted = 0;
        ...
        UssdMenu previousMenu = DatabaseHandler.getRequestedMenu(previousBreadCrumb, displayedMenu.getCategoryId());
        ...
        if (parts.length > 1) {
            ...
            previousMenu.setTitle(DatabaseHandler.getCategoryNameFromId(displayedMenu.getCategoryId()).replace("_", " "));
        }
     ...
     * @throws SQLException
     */
    public UssdResponse getNextMenu(UssdRequest request, UssdMenu previousMenu) throws Exception {
        isCompleted = 0;
        UssdResponse appResp = new UssdResponse();
        ...
        if (categoryId == 0) {
            ...
            categoryId = DatabaseHandler.getCategoryIdFromName(selectedItem);
            selectedItem = selectedItem.replace("_", " ");
        }
        else {
        ...
        UssdMenu currentMenu = DatabaseHandler.getRequestedMenu(previousMenu.getBreadCrumb(), categoryId);
        ...
        if (currentMenu.getItemCount() == 0) {
            ...
            appResp = getContent(request, currentMenu.getBreadCrumb(), categoryId);
